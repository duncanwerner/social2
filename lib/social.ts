...
export type Player = Branded<number, "Player">
...
/** utility */
export const CreatePlayer = (id: number) => id as Player;
 ...
 */
const RandomRound = (players: Player[], courts: number): Round => {
...
const RandomRounds = (count: number, players: Player[], courts: number): Round[] => {
  const rounds: Round[] = [];
  for (let i = 0; i < count; i++) {
 ...
 * @param options - 
 */
export const NextRound = (players: Player[], courts: number, previous_rounds: Round[] = [], options: Partial<Options> = {}): InstrumentedRound => {

  // force sitting: rmove player(s) from the pool, then 
  // add them back at the end

  const restore_players: Player[] = [];
  ...
  const sitting: Map<Player, number> = new Map();
  const opponents: Map<Player, Map<Player, number>> = new Map();

  // for absences, cap on "free sitting"
  const absence_sit_cap = 2;
  const absence_map: Map<Player, number> = new Map();
    ...
    for (const matchup of round.matchups) {
      ...
      for (const a of matchup.A) {
        for (const b of matchup.B) {
          ...
          const check: Map<Player, number> = opponents.get(key) || new Map();
  ...
  console.info("P", players.length, "N", n);
